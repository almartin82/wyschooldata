...
```{r hispanic-chart}
ggplot(hispanic_trend, aes(x = end_year, y = pct_hispanic)) +
  geom_line(linewidth = 1.2, color = "#FF6B35") +
  geom_point(size = 3, color = "#FF6B35") +
  scale_y_continuous(limits = c(0, NA)) +
  labs(
    title = "Hispanic Student Share in Wyoming (2008-2024)",
    subtitle = "Fastest-growing demographic in Wyoming schools",
    x = "School Year",
    y = "Percent Hispanic"
  )
```
...
```{r sweetwater-chart}
ggplot(sweetwater, aes(x = end_year, y = total)) +
  geom_line(linewidth = 1.2, color = "#4169E1") +
  geom_point(size = 3, color = "#4169E1") +
  scale_y_continuous(labels = scales::comma) +
  labs(
    title = "Sweetwater County (Rock Springs/Green River) Enrollment",
    subtitle = "Natural gas and trona mining drive enrollment cycles",
    x = "School Year",
    y = "Total Enrollment"
  )
```
...
```{r econ-chart}
ggplot(econ_trend, aes(x = end_year, y = pct_econ)) +
  geom_line(linewidth = 1.2, color = "#8B0000") +
  geom_point(size = 3, color = "#8B0000") +
  scale_y_continuous(limits = c(0, NA)) +
  labs(
    title = "Economically Disadvantaged Students in Wyoming",
    subtitle = "Over 35% of students qualify for free/reduced lunch",
    x = "School Year",
...
```{r grade-chart}
ggplot(grade_dist, aes(x = grade_level, y = n_students, fill = grade_level)) +
  geom_col(show.legend = FALSE) +
  scale_y_continuous(labels = scales::comma) +
  scale_fill_viridis_d(option = "turbo") +
  labs(
    title = "Wyoming Enrollment by Grade (2024)",
    subtitle = "Fewer students in upper grades reflects mobility and demographics",
    x = "Grade Level",
