...
**NOT in tidy enrollment:** All subgroups are conditional on their
presence in the raw Wyoming WDE data.
...
**Note:** Raw column names are mapped to standard codes (`PK`, `K`,
`01`-`12`) in tidy output. Always filter on the mapped values.

### entity flags

`is_state`, `is_district`, `is_school`

Determined by the `type` column: `"State"`, `"District"`, `"School"`.
Note: Wyoming uses `is_school` instead of `is_campus`.
