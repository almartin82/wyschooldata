# ==============================================================================
# LIVE Pipeline Tests for wyschooldata
# ==============================================================================
#
...
test_that("Wyoming DOE website is accessible", {

  skip_if_offline()
  skip("TODO: Add Wyoming DOE URL and verify HTTP 200")
})
...
test_that("Can download Wyoming enrollment data file", {
  skip_if_offline()
...
test_that("Can parse Wyoming enrollment file", {
  skip_if_offline()
...
test_that("wyschooldata data file has expected columns", {
...
test_that("get_raw_enr returns data for valid year", {
  ...
  # Get available years
  years_info <- wyschooldata::get_available_years()
  ...
  tryCatch({
    raw <- wyschooldata:::get_raw_enr(test_year)
...
test_that("get_available_years returns valid year range", {
  result <- wyschooldata::get_available_years()
...
test_that("fetch_enr returns data with no Inf or NaN", {
  ...
  tryCatch({
    ...
    data <- wyschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Enrollment counts are non-negative", {
  ...
  tryCatch({
    ...
    data <- wyschooldata::fetch_enr(test_year, tidy = FALSE)
...
test_that("tidy=TRUE and tidy=FALSE return consistent totals", {
  ...
  tryCatch({
    ...
    wide <- wyschooldata::fetch_enr(test_year, tidy = FALSE)
    tidy <- wyschooldata::fetch_enr(test_year, tidy = TRUE)
...
test_that("Cache functions exist and work", {
  ...
  tryCatch({
    path <- wyschooldata:::get_cache_path(2024, "enrollment")
